 ...
 */
class MyNewWizardNewPage implements ISelectionChangedListener {
	...
	private MyNewWizardSelectionPage page;
	 ...
	 * @param projectsOnly
	 */
	public MyNewWizardNewPage(MyNewWizardSelectionPage mainPage,
	...
	protected FilteredTree createFilteredTree(Composite parent) {
		...
		MyDelegatingLabelProviderWithTooltip delegatingLabelProviderWithTooltip = new MyDelegatingLabelProviderWithTooltip(
		...
		treeViewer.setLabelProvider(delegatingLabelProviderWithTooltip);
		treeViewer.setComparator(MyNewWizardCollectionComparator.INSTANCE);
	...
	public void selectionChanged(SelectionChangedEvent selectionEvent) {
		...
		StructuredSelection ss=(StructuredSelection) selectionEvent.getSelection();
		WorkbenchWizardElement list=(WorkbenchWizardElement) ss.getFirstElement();
		try {
			//通过反射获取私有属性值
			Field field = list.getClass().getDeclaredField("id");
			field.setAccessible(true);//这句话必须要加 
			name=field.get(list);
			System.out.println("name"+field.get(list));
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(name.equals("importWizard")){
			page.setMessage("创建一个import文件");
		}
		if(name.equals("test.wizard1")){
			page.setMessage("创建一个新的wizard文件");
		}
